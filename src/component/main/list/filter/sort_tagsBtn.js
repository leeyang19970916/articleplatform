...
const SortTagsBtn = (props) => {
    ...
    if (status === "offcanvas") {
        return (<Fragment>
            <button type="button" className="btn btn-primary px-2 py-0 me-3  mb-3 tag-btn">{item.name} <a href="#" className="text-secondary" onClick={removeTagHandler}>x</a></button>
        </Fragment>)
    } else {
        return (<Fragment>
